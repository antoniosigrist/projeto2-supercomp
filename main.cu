...
__global__ void render_init(vec3 *fb,int max_x, int max_y,int ns,camera **cam, hitable **world, curandState *rand_state) {
...
int main() {
    ...
    render_init<<<blocks, threads>>>(fb, nx, ny, ns, d_camera, d_world, d_rand_state); //cria o kernel de tamanho block x threads e inicializa cada thread com o random state gerado pela
