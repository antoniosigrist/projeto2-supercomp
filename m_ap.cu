...
__global__ void rand_init(curandState *rand_state,hitable **d_list, hitable **d_world, camera **d_camera, int nx, int ny) {
...
int main() {
    ...
    rand_init<<<1,1>>>(d_rand_state2,d_list, d_world, d_camera, nx, ny);  // inicializa kernel que cria seed no bloco 0 thread 0
